...
public class Controller4 implements Initializable {
    ...
    public void ajouter() throws  IOException{
        Parent ajou = FXMLLoader.load(getClass().getResource("../gestionSession/ajouterSsession.fxml"));
        client.getChildren().setAll(ajou);
        TrayNotification tray = new TrayNotification( );
        AnimationType type = AnimationType.SLIDE ;
        tray.setAnimationType(type);
        tray.setTitle("Information");
        tray.setMessage(" Service ajouter session");
        tray.setNotificationType(NotificationType.INFORMATION);
        tray.showAndDismiss(Duration.millis(3000));
    }
    public void rechercher() throws  IOException{
        Parent ajou = FXMLLoader.load(getClass().getResource("../gestionSession/rechercherSession.fxml"));
        client.getChildren().setAll(ajou);
        TrayNotification tray = new TrayNotification( );
        AnimationType type = AnimationType.SLIDE ;
        tray.setAnimationType(type);
